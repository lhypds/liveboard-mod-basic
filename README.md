...
Dragging on the empty sheet draws a dashed box round however many it is pulled over — anything it touches is picked up, not only what it swallows whole — and dragging any one of that group then moves all of it, as one offset applied to every box and one step on the Undo stack. Delete takes the whole group with its arrows.
A picked box thickens by half a pixel — 1.5px against the 1px hairline the rest are drawn at — and puts its handles out: round ones on the edges to pull arrows from, square ones on the corners to resize by, which hold the opposite corner where it is and stop at a minimum rather than turning the box inside out. A picked arrow, having no corners to put handles on, is drawn at that same 1.5px and nothing else. Nothing here is marked by a colour: the card is black on white throughout.
...
While the chart has the focus, `N` adds a box on its own (where Tab adds one on the end of the chain), `D` removes the selection as `Delete` does, `Enter` opens the one selected box to be typed into and `Escape` drops the selection. The letters are bare only — `Cmd+N` opens a browser window and `Ctrl+D` bookmarks the page, and neither is the card's to take.
